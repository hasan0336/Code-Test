...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        // Return an empty array if user not found
        if (!$cuser) return [];

        // Determine user type and get jobs accordingly
        if ($cuser->is('customer')) {
            ...
            // Handle other user types or return empty if not customer or translator
            return [];
        }

        // Separate jobs into emergency and normal jobs
        return $this->categorizeJobs($jobs, $user_id);
    }

    private function getCustomerJobs($customer)
    {
        // Fetch jobs for a customer with necessary relationships and filters
    ...
    private function categorizeJobs($jobs, $user_id)
    {
        ...
        // Add user check data to normal jobs
...
//            $jobs['total'] = $totaljobs;
    ...
    public function store($user, $data)
    {
        ...
        //            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
                ...
                $this->logger->error('Error in store method: ' . $e->getMessage());
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function updateJob($id, $data, $cuser)
    {
        try {
                ...
                // Handle the exception (log it, return a response, etc.)
                // Log the error message
                $this->logger->error('Error in updateJob method: ' . $e->getMessage());
